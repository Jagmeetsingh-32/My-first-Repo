...
export default function Navbar() {
    ...
    <>
      <nav className='relative px-8 h-16 w-full flex justify-between items-center bg-amber-200'>
        <div className='flex items-center gap-20'>
          {/* Logo */}
          <div className='justify-around text-xl font-bold'>Logo</div>
          ...
          <div className="hidden md:flex items-center  justify-between gap-8">
            ...
            {/* Login */}
        <button className=' hidden md:flex text-xl  bg-amber-50 hover:bg-blue-500'>
          Login
        </button>
  ...
  animate={{ opacity: open ? 1 : 0, scaleY: open ? 1 : 0.95 }}
  transition={{ duration: 0.35, ease: [0.22, 1, 0.36, 1] }}
  className="absolute left-0 top-full w-full md:hidden flex flex-col
            bg-amber-200 py-4 gap-4 shadow-md">
            <a className='text-xl pl-3 hover:text-blue-600'
              href="#">Login</a>
            <a className='text-xl pl-3 hover:text-blue-600'
              href="#">Home</a>
            <a className='text-xl pl-3 hover:text-blue-600'
